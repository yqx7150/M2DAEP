function map = multich_MDAEP_SR_modified_1change(degraded, up_scale, params, params2, map)
%
% Input:
...
for iter = 1:params.num_iter
    ...
    for iiii = 1:repeat_num
        if size(map,3) == 6
            ...
            map_all = repmat(map,[1,1,2]); % Switch channels for caffe
            input = map_all(:,:,[3,2,1,6,5,4]);
        end
        ...
        if size(map,3) == 6
            ...
            output_1 = output_1(:,:,[3,2,1]);
            output_2 = output_2(:,:,[3,2,1]);
        end
    ...
    if mod(iter,40)==0, figure(2000+iter);imshow([uint8(map)],[]);end
